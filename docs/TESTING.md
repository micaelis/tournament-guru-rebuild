...
| `email-change-flow` | The email-change confirmation legs (S12.12), Supabase mocked: the real `/auth/callback` GET routes the first click (message, no code) → `/email-change?stage=partial`, an expired link (`error_*`) → `?stage=error`, the second click (code exchange) → `/email-change`, a cross-device second click (`pkce_code_verifier_not_found`) → `/email-change` (done-but-signed-out, NOT error), any other exchange failure → `?stage=error`; non-email-change legs keep `/login?error=callback` + role/onboarding routing. The real `updateEmail` action: invalid + unchanged-address rejection before Supabase, the `emailRedirectTo → /auth/callback?next=/email-change` contract, and error surfacing. The two mailed-link legs against real GoTrue (inbucket) stay manual — see SMOKE-TESTS. |
