...
| `search-id-batching` | `searchEvents` with a facet matching 260 events (past the ~200-UUID URI limit) resolves with the full count: sort keys fetched per 150-id chunk, ordered app-side (premium first, then the sort key, created_at/id tie-breaks); date and rating (DESC NULLS LAST) sorts tile the full order exactly with no duplicates or gaps; sort-key ties fall through to created_at DESC then id ASC, and the app-side order matches the SQL path's ORDER BY page-for-page over the same tied fixture; intersection semantics survive the batching (a second facet narrows, an empty intersection stays empty). Mutation-verified three ways: the pre-fix single `.in()` ("URI too long"), a dropped comparator id tie-break, and a dropped SQL id ORDER BY each fail the probe (S11.7). |
...
| `h1-public-views` | Public reviewer/host identity reads go through DEFINER views; no attendee PII (last_name, email, DOB) leaks on a direct table read. The attendee name rule (S11.5): `last_initial` is served by `review_author_public` and `public_attendees` while selecting `last_name` on them (and on `public_comment_authors`) errors; `public_attendees` excludes EDs and blocked users. The ED rule (S11.8): `public_directors` serves the ED's full name (EDs are public business identities; it feeds the event-page host row) while `public_event_owners` stays a lean host-logo lookup that refuses `last_name`. Also asserts the views are **read-only**: anon and authenticated-non-owner cannot INSERT/UPDATE/DELETE through any of the five `public_*` views, and no write reaches `profiles` (S8.5). |
...
| `reauth-delete` | Account deletion = true-delete + recompute (Option B): re-auth gate, the user's review/comment rows removed, event score recomputed, profile scrubbed + blocked, platform counter frozen. ED path included. Mutation-verified (anonymize-style row survival → red). |
...
| `signup-verify-redirect` | Post-signup routing off the real `signupAction` (Supabase client mocked): no session → `/signup/verify-email`; session (local dev) → `/onboarding`; invalid input → field errors, no redirect; terms unchecked/tampered → server-side rejection (S11.2). Dup-email guard (S12.4): Supabase's existing-email shape (user with an EMPTY `identities` array, no session) returns the `email` field error + `code: "email_exists"` and provably does NOT redirect to verify-email, while a populated `identities` still routes a genuine signup there. Mutation-verified: removing the identities guard sends the dup attempt to verify-email and fails the probe. |
| `event-validation` *(pure unit, `tests/`)* | The draft-vs-publish field rules extracted to `event-validation.ts` (S12.5): an all-blank "+ Add sponsor" scaffold row never validates for ANY intent (the "Save as draft → sponsor logo is invalid" false positive), drafts are never blocked by sponsor rules at all (only the title is mandatory), while publish AND update reject incomplete rows and unsafe URLs. Mutation check: reintroducing per-blank-row validation or de-gating the draft intent flips it red. No DB — pure function. |
| `notif-prefs` *(pure unit, `tests/`)* | The notification-prefs FormData contract extracted to `notif-fields.ts` (S12.10): a rendered-but-unchecked switch writes `false` (the "uncheck doesn't persist" half of the save bug), unrendered sections stay out of the patch (an attendee save can't wipe ED-only prefs), an empty form no-ops, and only the literal `on` counts as checked. Mutation check: `has()` instead of `=== "on"`, or dropping the `section:` gate, flips it red. No DB — pure function. |
| `email-change-flow` | The email-change confirmation legs (S12.12), Supabase mocked: the real `/auth/callback` GET routes the first click (message, no code) → `/email-change?stage=partial`, an expired link (`error_*`) → `?stage=error`, the second click (code exchange) → `/email-change`, a cross-device second click (`pkce_code_verifier_not_found`) → `/email-change` (done-but-signed-out, NOT error), any other exchange failure → `?stage=error`; non-email-change legs keep `/login?error=callback` + role/onboarding routing. Open-redirect hardening (S12.15): `next` is an exact allow-list — four hostile shapes (`/\evil.com`, `/\/evil.com`, `//evil.com`, `https://evil.com`) all stay on-origin with a valid code AND on the code-less leg, while the allow-listed `/reset/update` still routes; mutation-checked (the old `startsWith` prefix guard flips the two backslash cases red). The real `updateEmail` action: invalid + unchanged-address rejection before Supabase, the `emailRedirectTo → /auth/callback?next=/email-change` contract, and error surfacing. The two mailed-link legs against real GoTrue (inbucket) stay manual — see SMOKE-TESTS. |
...
| `mutations` | Profile edit, admin banned-word add, reaching the Add Event form; draft save succeeds with an untouched "+ Add sponsor" scaffold row left in the form (the S12.5 repro); full publish drives the masked mm/dd/yyyy date inputs (S12.7). |
...
| `notifications-save` | The S12.10 save-reset regression through the real form + Server Action: uncheck a switch, save, and assert all three layers — the DB row persisted `false`, the on-screen switch still shows unchecked right after the save (the old checkbox UI snapped back via React's post-action form reset), and a fresh page load agrees; the still-checked pair partner keeps `true`. Mutation-verified both halves: the pre-fix checkbox Switch failed the post-save UI assert against the genuine desync, and an always-`on` hidden input fails the DB assert. |
| `favorites` | Favoriting an event (add/remove). |
| `a11y` | Accessibility checks (labeled controls, keyboard reachability). |
| `uploads` | Real browser → Supabase Storage upload (S10.6): picks a PNG in the event form's logo field and asserts the field fills with a `/event-images/<ed-uid>/` public URL — proof the client upload + owner-scoped RLS path works, not just that the widget renders. Storage RLS is proven separately by the `storage-rls` probe; the promo-CSV upload rides the `mutations` promo-CSV spec. Both file-pick sites go through `helpers/hydration.setInputFilesHydrated` — route-level `loading.tsx` (S12.13) streams the page body in a deferred Suspense chunk, so a change event fired instantly after `goto` can land before React attaches the handler; the helper re-fires until the UI reacts (a real user's OS file-picker dwell time makes the race unhittable manually). |
