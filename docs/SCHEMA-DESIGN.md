...
- **promo_email_eligibility(text[])** (S10.12, capped S10.15): host-gated definer bridge for
  ...
  in input order. Input capped at 1000 addresses (the CSV row cap) — oversize raises 22023
  rather than truncating. Null-uid raise + `is_event_host()` + pinned search_path; EXECUTE
