 ...
 * locked-in off-white treatment: #f8fafc fill, 1px ink border, ink
 * label AND icon; hover goes a hair darker with a 1px lift + soft
 * shadow — a real but subordinate action (Duplicate, Share, Save as
 * draft, the section-header "+ Add X" CTAs). The legibility comes
 * from the full-strength ink border, so it holds up on white cards
 * and the slate page wash alike (client call, supersedes the S12.31
 * ink-outline tier). Accent = the brand-red fill, reserved for
...
export function Button({
  ...
  const variants: Record<Variant, string> = {
    ...
    secondary:
      "border border-slate-900 bg-slate-50 text-slate-900 hover:bg-[#eef2f7] hover:-translate-y-px hover:shadow-[0_6px_14px_-8px_rgba(15,23,42,.35)] active:translate-y-0 active:shadow-none disabled:hover:translate-y-0 disabled:hover:shadow-none focus-visible:ring-2 focus-visible:ring-slate-900/25",
    accent:
