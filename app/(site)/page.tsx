...
/* ─── HighlightSwipe — hand-drawn marker underline from the prototype.
   Brand-disciplined: defaults to a red tint. ─── */
...
export default async function HomePage() {
  // Stats are cheap count queries — fetch once, share with the hero + manifesto.
  // Popular chips come from real logged searches (falls back to defaults).
  const [{ data: stats }, popularSearches] = await Promise.all([
    getStats(),
    getPopularSearches(),
  ]);
  ...
  if (error || events.length === 0) {
    return (
      <EmptyState
        ...
        detail={error ? `${error}` : "Events with premium = true will appear here."}
      />
    );
  }
