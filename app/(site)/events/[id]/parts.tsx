...
export function EventDetail({
  ...
  ownReviews: EventReviewRow[];
  fallbackReviews: EventReviewRow[];
  usingFallback: boolean;
  ageGroups: EventAgeGroupRow[];
  sponsors: SponsorRow[];
  otherEvents: EventRow[];
  director: DirectorProfile | null;
  parentProfile: EventProfileSummary | null;
}) {
  const [contactOpen, setContactOpen] = useState(false);

  const concluded = eventConcluded(event.status, event.end_date);
  const reviews = ownReviews.length ? ownReviews : fallbackReviews;
...
function ReviewsSection({
  ...
  const coach = useMemo(() => reviews.filter((r) => r.kind === "coach"), [reviews]);
  const attendee = useMemo(
    () => reviews.filter((r) => r.kind === "attendee"),
    [reviews],
  );
  ...
  const filtered = useMemo(() => {
    ...
    out = [...out];
    if (sort === "top") {
      out.sort((a, b) => numOr0(b.overall_rating) - numOr0(a.overall_rating));
    } else {
      out.sort(
        (a, b) =>
  ...
  return (
    <section id="reviews" className="flex flex-col gap-4">
      ...
      {usingFallback && parentProfile && (
        <FallbackNote parent={parentProfile} />
      )}
          ...
          <Card>
            ...
            {filtered.length === 0 ? (
              ...
              >
                {filtered.map((r) => (
                  <li key={r.id}>
                    <ReviewItem review={r} showEventTitle={usingFallback} />
                  </li>
                ))}
              </ul>
            )}
...
function FallbackNote({ parent }: { parent: EventProfileSummary }) {
  return (
    <div
    ...
    >
      ...
      <div style={{ fontSize: 13, lineHeight: 1.5 }}>
        <b>Reviews from past editions of {parent.title || "this tournament"}.</b>{" "}
        This upcoming event doesn&apos;t have its own reviews yet — the ratings
        below come from prior years so you can gauge what to expect.
      </div>
    </div>
  );
}
...
function ReviewItem({
  ...
  const rating = numOr0(review.overall_rating);
  const isCoach = review.kind === "coach";
    ...
    >
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div className="flex min-w-0 items-center gap-2.5">
          <Avatar name={review.username || "Anonymous"} size={38} />
          <div className="min-w-0">
            <div className="flex flex-wrap items-center gap-1.5">
              ...
              <VerifiedPill />
              {isCoach ? <CoachPill /> : <AttendeePill />}
              {review.guru_review && <GuruPill />}
            </div>
            <div
              className="mt-0.5 truncate"
              style={{ fontSize: 11.5, color: "var(--color-text-muted)" }}
            >
              ...
              {fmtFullDate(review.created_at)}
              {showEventTitle && review.event_title
                ? ` · ${review.event_title}`
                : ""}
            </div>
          </div>
...
function VerifiedPill() {
  return (
    <span
      className="font-heading inline-flex items-center gap-1 rounded-full uppercase"
      style={{
        fontSize: 9.5,
        fontWeight: 800,
        letterSpacing: ".08em",
...
function ContactPanel({
  ...
  return (
    ...
    >
      ...
      <div className="mt-4">
        <InfoTip>
          Questions about brackets, housing, or schedule? Message the host
          directly and they&apos;ll reply through Tournament Guru.
        </InfoTip>
      </div>
    </div>
  );
...
/* ───────────────────────────────────────────────────────────────────
   Contact host modal — collects a message; a real submit endpoint
   isn't wired yet (there's an event_requests table but the brief
   forbids inventing endpoints). Submits log to the console for now
...
function avgRating(reviews: EventReviewRow[]): number {
  if (reviews.length === 0) return 0;
  const sum = reviews.reduce((a, r) => a + numOr0(r.overall_rating), 0);
  return sum / reviews.length;
}

function distribution(reviews: EventReviewRow[]): number[] {
  const buckets = [0, 0, 0, 0, 0]; // 5-star, 4, 3, 2, 1
  for (const r of reviews) {
    const v = numOr0(r.overall_rating);
    if (v <= 0) continue;
