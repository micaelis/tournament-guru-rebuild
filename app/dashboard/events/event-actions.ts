 ...
 * every mandatory field + end_date >= start_date. Sponsor rows are
 * validated only for publish/update — a draft saves in-progress rows
 * as-is, and all-blank scaffold rows are dropped before validation
 * (see event-validation.ts).
 *
...
export async function saveEvent(
  ...
  // Child collections come across as JSON blobs. Blank sponsor rows
  // ("+ Add sponsor" scaffolding the ED never filled) are dropped here
  // so they neither validate nor save.
  const ageGroups = parseJson<AgeGroupInput[]>(formData.get("age_groups")) ?? [];
  const sponsors = meaningfulSponsors(
    parseJson<SponsorInput[]>(formData.get("sponsors")) ?? [],
  );
  ...
  // Draft requires only a title; publish enforces everything; sponsor
  // rows gate only the live surface (publish/update). The pure rules
  // live in event-validation.ts so tests can pin them without a DB.
  const fieldErrors = collectEventFieldErrors({
    intent,
    base,
    ageGroups,
    sponsors,
